...
public class DenyARequest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int successful = reimbursment.update(employeeId, managerId, reimbursmentRequestId, "deny");
		PrintWriter writer = response.getWriter();
		request.getRequestDispatcher("denyARequest.html").include(request, response);
		if (successful == 0) {
			writer.println("<h1>This employee's reimbursment request has been successfully denied!</h1>");
		} else if (successful == -1) {
			writer.println(
					"<h1>Sorry, an error occured while denying this employee's reimbursement request. Please try again.</h1>");
			...
			writer.println("<h1>You do not manage this employee. You cannot deny their requests.</h1>");
		}
	}
